...
export default function ActivationPage() {
  ...
  const handleInscrire = async () => {
    ...
    if (!/^\d{7}[A-Z]$/.test(matricule)) {
      setError('Matricule invalide — format attendu : 7 chiffres + 1 lettre (ex : 0525247O).');
      return;
    }
    if (!dateNaissance) {
        ...
        <div className="flex w-full rounded-2xl overflow-hidden border border-white/25 bg-white/10 backdrop-blur-sm">
          {(['inscription', 'connexion'] as Mode[]).map((m) => (
            <button
              ...
              style={{
                background: mode === m ? 'rgba(255,255,255,.25)' : 'transparent',
                color: '#fff',
        ...
        {mode === 'inscription' && (
          <div className="w-full flex flex-col gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] opacity-60 uppercase tracking-widest">Vérification</span>
              <div className="flex-1 h-px bg-white/25" />
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div>
                ...
                <input
                  className={INPUT_CLS + ' tracking-widest text-center font-bold text-lg'}
                  placeholder="0525247O"
                  maxLength={8}
                  value={matricule}
                  onChange={(e) => setMatricule(e.target.value.toUpperCase())}
                />
                <p className="text-[10px] opacity-65 mt-1">7 chiffres + 1 lettre · délivré par la Nation</p>
              </div>
            ...
            <Button
              ...
              disabled={loading || !nom.trim() || !prenoms.trim() || matricule.length < 8 || !dateNaissance || password.length < 8}
            >
